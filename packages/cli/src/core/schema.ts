...
function checkType(value: unknown, schema: Schema, path: string): void {
    ...
    case "array":
      ...
      if (ok && schema.items) {
        (value as unknown[]).forEach((item, i) =>
          checkType(item, schema.items as Schema, `${path}[${i}]`),
        );
      }
      break;
    case "object":
  ...
  if (typeof value === "number") {
    if (schema.maximum !== undefined && value > schema.maximum) {
      throw new OmtError("INVALID_INPUT", `'${path}' must be <= ${schema.maximum}`);
    }
    if (schema.minimum !== undefined && value < schema.minimum) {
      throw new OmtError("INVALID_INPUT", `'${path}' must be >= ${schema.minimum}`);
    }
  }
}
...
export function validateInput(
  ...
  for (const key of schema.required ?? []) {
    if (out[key] === undefined || out[key] === null) {
      throw new OmtError("INVALID_INPUT", `missing required field '${key}'`);
    }
  }

  for (const key of Object.keys(out)) {
    if (props[key]) {
      checkType(out[key], props[key], key);
    }
  }
